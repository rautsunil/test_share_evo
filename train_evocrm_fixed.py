...
class EvoCRMDataset(Dataset):
    ...
    def __init__(self, data_dir: str, split: str = "train",
        ...
        # Targets — with NaN guard (NaN targets cause CUDA assert in BCE loss)
        self.targets = {}
        for col in ["churn", "upsell", "early_adopter", "satisfaction_risk"]:
            if col in merged.columns:
                vals = pd.to_numeric(merged[col], errors="coerce").fillna(0).values
                vals = np.clip(vals, 0, 1).astype(np.float32)  # Ensure strictly {0, 1}
                self.targets[col] = torch.FloatTensor(vals)
        ...
        self.max_product_id = 1000  # Default
        if seq_path.exists():
            with open(seq_path) as f:
                raw_seqs = json.load(f)
            self.sequences = self._build_sequence_tensors(raw_seqs, merged["user_id"].values)
            # Detect max product ID for embedding table sizing
            self.max_product_id = int(self.sequences[:, :, 1].max().item()) + 1
        else:
            self.sequences = torch.zeros(len(merged), max_seq_length, 3)

        self.n_features = self.features.shape[1]
        print(f"    Max product ID in sequences: {self.max_product_id}")
...
class InteractionTower(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256, n_event_types: int = 20,
        ...
        self.product_embed = nn.Embedding(n_products + 1, 64, padding_idx=0)
...
class EvoCRMModel(nn.Module):
    ...
    def __init__(self, n_features: int, embed_dim: int = 256,
                 ...
                 n_products: int = 50000, tasks: Dict[str, str] = None):
        super().__init__()
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, predictions: Dict, targets: Dict) -> Tuple[torch.Tensor, Dict]:
        ...
        for task_name, pred in predictions.items():
            ...
            # Guard: skip if target has NaN (prevents CUDA assert)
            ...
            if task_type == "binary":
                # Clamp BOTH pred and target for BCE safety
                pred_safe = pred.clamp(1e-7, 1 - 1e-7)
                target_safe = target.clamp(0.0, 1.0)
                loss = nn.functional.binary_cross_entropy(
                    pred_safe, target_safe, reduction="mean"
                )
            else:
                loss = nn.functional.huber_loss(pred, target, reduction="mean", delta=1.0)

            # Guard: skip if loss is NaN or Inf
...
def run_pipeline(args):
    ...
    n_products = max(train_ds.max_product_id, 100)  # From dataset, with safety floor
    print(f"  Event types: {n_event_types} | Products in sequences: {n_products}")
